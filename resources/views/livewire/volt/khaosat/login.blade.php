...
state(['email' => '', 'password' => '', 'remember' => false, 'showSurveyForm' => false, 'surveyDate' => '', 'selectedMarket' => '', 'currentSurvey' => null, 'prices' => []]);

$markets = computed(fn() => Market::where('active', true)->orderBy('order')->get());

$products = computed(fn() => Product::where('active', true)->where('is_default', true)->orderBy('order')->get());
...
$login = function () {
    ...
    if (auth('sale')->attempt(['email' => $this->email, 'password' => $this->password], $this->remember)) {
        $sale = auth('sale')->user();
        if (!$sale->active) {
            auth('sale')->logout();
        ...
        return redirect()->intended(route('khaosat'));
    }
...
$viewHistory = function () {
    // TODO: Navigate to survey history
    session()->flash('message', 'Chức năng xem lịch sử sẽ được triển khai tiếp theo.');
};

$createSurvey = function () {
    $this->validate(['surveyDate', 'selectedMarket']);

    $survey = Survey::create([
        'sale_id' => auth('sale')->id(),
...
$loadProducts = function () {
    $products = Product::where('active', true)->where('is_default', true)->orderBy('order')->get();
    $this->prices = [];
...
@if(auth('sale')->check())
            ...
            <div class="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        ...
                        <div class="flex items-center">
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="text-lg font-medium text-gray-900">Chọn ngày và chợ để bắt đầu</dd>
                                </dl>
                            </div>
                        </div>
                        <div class="mt-5">
                ...
                <div class="bg-white overflow-hidden shadow rounded-lg">
                    <div class="p-5">
                        ...
                        <div class="mt-5">
                            <div class="-ml-2 -mt-2 flex flex-wrap items-bottom">
                                <flux:button variant="secondary" wire:click="viewHistory" class="ml-2 mt-2">Xem lịch sử</flux:button>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    @if($showSurveyForm)
    <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" id="survey-modal">
            ...
            <div class="mt-3">
                ...
                <form wire:submit="createSurvey">
                    ...
                    <div class="flex justify-end space-x-2">
                        <flux:button type="button" wire:click="$set('showSurveyForm', false)" variant="secondary">
                            Hủy
                        </flux:button>
        ...
        <div class="bg-white shadow overflow-hidden sm:rounded-md">
            <div class="px-4 py-5 sm:px-6">
                <h3 class="text-lg leading-6 font-medium text-gray-900">
                    Nhập giá sản phẩm - {{ $currentSurvey->market->name }} ({{ $currentSurvey->survey_day }})
                </h3>
            ...
            @error('prices')
                ...
                <form wire:submit="savePrices" class="px-4 py-5 sm:p-6">
                    ...
                    <div class="mt-6 flex justify-end space-x-3">
                        <flux:button type="button" wire:click="$set('currentSurvey', null); $set('prices', [])" variant="secondary">
                            Hủy
                        </flux:button>
                        <flux:button type="submit" variant="primary">
                            Lưu giá
